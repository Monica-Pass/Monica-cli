...
| 参数被拒 | `invalid_request` | 范围与操作组合不合法（api 类未用 `*`、与 Issue 混列）、TTL 越界、请求文件里有未知字段 | 见 [6.1](#61-范围与操作的互斥规则) |
...
| 服务端不给强 ETag | `remote_version_required` | 见第 7 节：改用 `publish` 新文件名，程序不会强推 |
