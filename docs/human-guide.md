...
| 分类 | `delete-category <分类ID>` | 里面还有条目或子分类 → `invalid_request`，并报出还差多少，绝不连带删除 |
...
| 参数被拒 | `invalid_request` | 范围与操作组合不合法（api 类未用 `*`、与 Issue 混列）、TTL 越界、请求文件里有未知字段；删除时还可能是键回的名称与目标不符、条目正被连接绑定、或分类非空 | 见 [6.1](#61-范围与操作的互斥规则)；删除的三种拒绝见 [5.1](#51-删除写的是墓碑) |
